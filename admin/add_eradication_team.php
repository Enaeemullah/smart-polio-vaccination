...
if(isset($_GET['er']) && !empty($_GET['er']) && $_GET['er']==1)
{
    ...
    $password = $_SESSION['eradication-data']['password'];
}



    if(isset($_POST['add_eradication_team']))
    {
        ...
        if(empty($name)||empty($cnic)||empty($city) || empty($address) ||empty($contact_number) || empty($password))
        {
            ...
            header("location:add_eradication_team.php?er=1");
            exit();
        }
        ...
        {
            $query ="INSERT INTO teams (type_id,name,cnic,city_id,address,contact_number,password,role_id) VALUES ('2','$name','$cnic','$city','$address','$contact_number','$password','2')";
            
            $result = mysqli_query($connection ,$query);
            if($result)
            {
                $_SESSION['info'] = "Sucessfully Added Eradication Team";
            ...
            else
            {
                $_SESSION['info'] = "Failed".mysqli_error($connection );
                   header("location:add_eradication_team.php");
            exit();
            }
        }
}
    ...
    <input type="text" name="password" id="" class="form-control" value="<?php if(isset($password)) echo $password; ?>">     
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        <div class="hvr">
             <input type="submit" name="add_eradication_team" class="btn btn-primary" value="Add Eradication Team">
                        </div>
                    </div>
                </div>
